...
def read_grid(file_path):
    ...
    for grid in new_file:
        grid_count.append(csv_line_to_list(grid))
    new_file.close()
    return grid_count
...
| 02/10 at 10:38 pm
Grading comment:
-0 you should combine these two if statements to make your code readable (and using 2 if-statements like this when only 1 is guaranteed to be true is stylistically bad as well)

        return 0
    if i >= len(pixel_grid) or j >= len(pixel_grid[0]):
        return 0
    else:
...
def blur(pixel_grid):
    ...
    for i in range(0, len(pixel_grid)):
        ...
        for j in range(0, len(pixel_grid[0])):
            new_grid.append(average_of_surrounding(pixel_grid, i, j))
        blurred_grid.append(new_grid)  # replaces blurred list w/ new list
    return blurred_grid
...
write_image(out_image_filename, blur(input_grid))
write_grid(out_grid_filename, blur(input_grid))
print("Program done")
