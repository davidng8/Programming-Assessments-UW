...
* CSE 160, Winter 2023
* Homework 4
* Description: Homework 4 Clusters
* Collaboration:
"""
...
def euclidean_distance(point1, point2):
    ...
    using the formula with square root.
    """
    d = 0.0
...
def get_closest_centroid(point, centroids_dict):
    ...
    This function calculates the closest centroid to a point using the function
    euclidean_distance.

    """
    centroids_list = list(centroids_dict.keys())[0]
    low_point = euclidean_distance(point, centroids_dict[centroids_list])
     nearest_point = ""
    for i in centroids_dict:
        point_distance = euclidean_distance(point, centroids_dict[i])
        if low_point >= point_distance:
            low_point = point_distance
            nearest_point = i  # string
    return nearest_point  # return string
# problem for students
def update_assignment(list_of_points, centroids_dict):
    ...
    This function assigns points in the list to the closest centroid.

    """

    points_dict = {}
    for points in list_of_points:
        nearest_point = get_closest_centroid(points, centroids_dict)
        if nearest_point not in points_dict:
            points_dict[nearest_point] = [points]
        else:
            points_dict[nearest_point].append(points)
    return points_dict
...
def mean_of_points(list_of_points):
    ...
    This function finds the mean of points and creates a new
    list of the mean of points.
    ...
    for i in range(len(list_of_points[0])):
        ...
        for j in range(len(list_of_points)):
            point_sum = point_sum + list_of_points[j][i]
        average = point_sum / len(list_of_points)
        list_mean.append(average)
    return list_mean
...
def update_centroids(assignment_dict):
    ...
    This function updates the dictionary of the centroids with the means of
    all data points in that cluster.
    """

    updated_dict = {}
    for (key, value) in assignment_dict.items():
        updated_dict[key] = mean_of_points(value)
    return updated_dict
